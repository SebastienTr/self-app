...
cmd_app_launch() {
  ...
  # 3b. Write .env for Metro (Expo reads .env at bundle time)
  local env_file="$ROOT_DIR/apps/mobile/.env"
  {
    echo "# Auto-generated by dev-tools.sh — do not edit manually"
  ...
  # 5b. Append pairing token to .env if available
  if [[ -n "${EXPO_PUBLIC_DEV_PAIRING_TOKEN:-}" ]]; then
    echo "EXPO_PUBLIC_DEV_PAIRING_TOKEN=${EXPO_PUBLIC_DEV_PAIRING_TOKEN}" >> "$env_file"
  fi
