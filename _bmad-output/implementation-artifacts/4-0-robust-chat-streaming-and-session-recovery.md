...
- [ ] Task 1: Extend LLMProvider Protocol with streaming support (AC: #8, #7)
  - [ ] 1.1 Add `LLMStreamChunk` dataclass to `apps/backend/app/llm/base.py`: `delta: str`, `accumulated: str`, `done: bool`, plus final metadata fields (`tokens_in`, `tokens_out`, `model`, `provider`, `latency_ms`)
  - [ ] 1.2 Add `stream(prompt: str) -> AsyncIterator[LLMStreamChunk]` method to `LLMProvider` Protocol
  - [ ] 1.3 Add default `stream()` implementation on `CLIProvider` base class that calls `execute()` and yields a single chunk (backward-compatible fallback for CLI providers)
  - [ ] 1.4 Implement `stream()` on `AnthropicAPI` using `client.messages.stream()` → `stream.text_stream` async iterator
  - [ ] 1.5 Implement `stream()` on `DeepSeekAPI` using `client.chat.completions.create(stream=True, stream_options={"include_usage": True})`
  - [ ] 1.6 Tests: `test_llm_streaming.py` — test chunk iteration, accumulated text, final metadata, fallback behavior for CLI providers

- [ ] Task 2: Decouple agent work from WebSocket with task manager (AC: #3, #4, #9)
  - [ ] 2.1 Create `apps/backend/app/task_manager.py` with `AgentTaskManager` class
  - [ ] 2.2 Implement per-session message buffer with monotonic sequence numbers (`PendingMessage` dataclass: `id`, `seq`, `payload`, `created_at`)
  - [ ] 2.3 Implement `buffer_message(session_id, payload)` — append to in-memory buffer with seq
  - [ ] 2.4 Implement `drain_buffer(session_id, after_seq)` — return messages after given seq
  - [ ] 2.5 Implement `ack_messages(session_id, up_to_seq)` — prune acknowledged messages
  - [ ] 2.6 Add TTL eviction (300s) — messages older than 5 minutes are dropped (client should full-sync instead)
  - [ ] 2.7 Implement per-session message queue for concurrent chat messages: new messages are queued and processed sequentially after the current agent task completes (no cancellation)
  - [ ] 2.8 Tests: `test_task_manager.py` — buffer CRUD, seq ordering, TTL eviction, multi-session isolation, concurrent message queuing

- [ ] Task 3: Modify agent.handle_chat for true streaming (AC: #1, #2, #7)
  - [ ] 3.1 Replace `await provider.execute(prompt)` with `async for chunk in provider.stream(prompt)`
  - [ ] 3.2 Send each `chat_stream` delta via task manager buffer (not direct `ws.send_json`)
  - [ ] 3.3 After stream completes, run module spec detection on `accumulated` text
  - [ ] 3.4 Send granular status updates at each pipeline phase: `thinking` → (tokens stream) → `discovering` → `composing` → `saving` → `idle`
  - [ ] 3.5 Handle `WebSocketDisconnect` gracefully — agent task continues, results buffered
  - [ ] 3.6 Maintain backward compatibility: if `stream()` yields a single chunk (CLI fallback), behavior is identical to current
  - [ ] 3.7 Tests: update `test_agent.py` and `test_agent_edge.py` for streaming paths

- [ ] Task 4: Modify WebSocket endpoint for writer loop and sync replay (AC: #3, #4, #9)
  - [ ] 4.1 Add writer loop to `/ws` endpoint: `asyncio.Task` that uses `await queue.get()` (push-based, no polling/sleep) to drain task manager buffer → `ws.send_json`
  - [ ] 4.2 On `chat` message: enqueue to session's message queue in task manager (sequential processing, no cancellation of in-flight work)
  - [ ] 4.3 On `sync` message: accept `last_seq` parameter, replay missed messages from buffer before module delta sync
  - [ ] 4.4 On `WebSocketDisconnect`: cancel writer task only (agent task continues)
  - [ ] 4.5 Add `seq` field to all server→client messages (envelope wrapping)
  - [ ] 4.6 Tests: update `test_ws.py` and `test_ws_edge.py` for seq, replay, and disconnect scenarios

- [ ] Task 5: Add sequence tracking and token buffering on mobile (AC: #4, #6)
  - [ ] 5.1 Add `lastSeq: number` to `connectionStore.ts` with `setLastSeq` action
  - [ ] 5.2 Update `wsClient.ts` `onmessage` handler to extract `seq` from all server messages and call `setLastSeq`
  - [ ] 5.3 Update `sendSyncOnReconnect()` to include `lastSeq` in sync payload
  - [ ] 5.4 Create `apps/mobile/services/streamBuffer.ts` with `bufferToken(delta)` and `flushImmediately()` using `requestAnimationFrame` batching
  - [ ] 5.5 Update `chatSync.ts` to use `bufferToken()` instead of direct `appendStreamDelta()` calls
  - [ ] 5.6 Add deduplication by `seq` in `chatSync.ts` (skip messages with already-seen seq numbers)
  - [ ] 5.7 Tests: `streamBuffer.test.ts`, update `chatSync.test.ts`, update `connectionStore.test.ts`

- [ ] Task 6: Add AppState lifecycle management on mobile (AC: #5)
  - [ ] 6.1 Create `apps/mobile/hooks/useAppStateConnection.ts` hook
  - [ ] 6.2 Listen to `AppState.addEventListener('change')` for `active`/`background`/`inactive` transitions
  - [ ] 6.3 On background → foreground (after >5s): trigger reconnect via `connect(wsUrl)`, sync fires automatically
  - [ ] 6.4 On foreground → background: no-op (let OS handle WS closure naturally, don't force disconnect)
  - [ ] 6.5 Wire hook into `App.tsx` (or tab navigator root)
  - [ ] 6.6 Tests: `useAppStateConnection.test.ts` — mock AppState, verify reconnect triggers

- [ ] Task 7: Extend AgentState type for granular status (AC: #2)
  - [ ] 7.1 Update `apps/mobile/types/ws.ts` — add `'streaming'` and `'saving'` to `AgentState` union
  - [ ] 7.2 Update `apps/mobile/components/shell/Orb.tsx` — handle new states with appropriate animations
  - [ ] 7.3 Update `apps/mobile/components/shell/ChatBubble.tsx` — streaming cursor replaces pulsing dots when `agentStatus === 'streaming'`
  - [ ] 7.4 Update `apps/backend/app/agent.py` — send `status: streaming` after first token, `status: saving` before DB write
  - [ ] 7.5 Tests: update Orb tests, ChatBubble tests, and chatSync tests for new states
